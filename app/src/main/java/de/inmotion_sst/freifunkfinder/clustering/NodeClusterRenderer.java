...
// This NodeClusterRenderer is based on the original ClusterRenderer and

public class NodeClusterRenderer<T extends Node> implements ClusterRenderer<T> {

    private boolean shouldAnimate;
    private final GoogleMap mMap;
    private final IconGenerator mIconGenerator;
    private final NodeClusterManager<T> mClusterManager;
    private final float mDensity;

    private static final int[] BUCKETS = {10, 100, 200, 500, 1000, 5000, 10000};
    ...
    protected String getClusterText(Cluster<T> cluster) {
        return Integer.toString(cluster.getSize());
    }
    ...
    /**
     * Transforms the current view (represented by NodeClusterRenderer.mClusters and NodeClusterRenderer.mZoom) to a
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(NodeClusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (NodeClusterRenderer.this.mClusters != null && shouldAnimate) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : NodeClusterRenderer.this.mClusters) {
            ...
            mMarkers = newMarkers;
            NodeClusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
    ...
    protected void onBeforeClusterItemRendered(T item, MarkerOptions markerOptions) {
        markerOptions
                .title(String.format("%s (%s)", item.getName(), item.getStatus()))
                .snippet(item.getCommunity());
    }
